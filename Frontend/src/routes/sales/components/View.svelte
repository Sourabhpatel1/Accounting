<script>
    ...
    const filter = ()=>{
        ...
        if (partyName) {
            let customerId
            try {
                customerId = customers.filter(cus=>cus.name === partyName)[0].id
            } catch (error) {
                
            } 
            if (customerId) {
                userInvoices = invoices.filter(inv=>inv.invoice.customer_id === customerId)
            }
        }
        if (startDate) {
...
{#if viewinginvoice}
    <ViewInvoice invoice={invoiceToView} {accounts} customers={customers} on:close={()=>{viewinginvoice=false; invoiceToView=null}}/>
{/if}
<main>
    <div class="filters">
