...
<main>
    <nav>
        <div class="tabs">
            <button class="{current==='new'?'active':''}" on:click={()=>{current='new'}}>New Invoice</button>
            <button class="{current==='view'?'active':''}" on:click={()=>{current='view'}}>View Invoices</button>
        </div>
        <div class="save">
            {#if current === 'new'}
    ...
    <div class="content">
        {#key data}
            ...
            {:else}
                <View invoices={data.invoices} accounts={data.accounts} vendors={data.vendors}/>
            {/if}
        {/key}
    </div>
</main>
